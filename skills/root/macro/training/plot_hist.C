void plot_hist(const char *filename, const char *make_fit)
{
    //--------------------------------------------------------------------
    // CANVAS
    //--------------------------------------------------------------------
    ...
    for(int i=1; i<9; i++)
    {
        ...
        while(1)
        {
            file >> value;
            hist->Fill(value);
            if(file.eof()) break; // Stop the while loop
        }
        file.close();
        
        if (strcmp(make_fit, "fit") == 0) // strcmp() == 0 to compare strings instead of a simple ==
        {
            //-------------------------------------------------------------------- 
            // FIT
            //--------------------------------------------------------------------
            ...
            */
            // Set the line width of fit function
            //fit->SetLineWidth(3);
            // Set the line color of the fit function
            fit->SetLineColor(kRed);
            // Set the line style of the fit function
            //fit->SetLineStyle(2); // 2 = --- (dash line)
            // Set fit paramters (sometimes on the fit function or maybe on my data,
            // the fit not converges immediatly. So, I have to say to root input parameters
            // Set the constant of gaus
            fit->SetParameter(0,40); // 1000 = maximum values of entries
            // Set the mean value
