   ...
   for (Long64_t jentry=0; jentry<nentries/2; jentry++) {
      ...
      for (Int_t chn=0; chn<16; chn++) {
         ...
         if (board165_hit[chn] == 1) {
            ...
            Double_t v_start_165;                              // V START OF SIGNAL [V]
            Double_t v_stop_165;                               // V STOP OF SIGNAL  [V]
            Double_t v_min_position_165 = 0;                   // POSITION OF V PEAK
            ...
            if (status==1) {
               ...
               for (Int_t i=50; i<800; i++) { // LOOP ON SAMPLES OF WAVEFORM
                  if (board165_waveform[chn][i] < v_peak_165) {
                     ...
                     v_min_position_165 = i;
                     t_peak_165 = board165_time[chn][i] * TMath::Power(10,9);
                  }
               }
               ...
               // TIME/V START
               for (Int_t i=10; ; i++) {
                  if ((v_base_165 - board165_waveform[chn][i]) > 5 * sigma_v_base_165) {
                     t_start_165 = board165_time[chn][i-1] * TMath::Power(10,9); //[ns]
                     v_start_165 = i-1; // [V]
                     break;
                  }
               }

               // TIME/V STOP (v_min_position_165)
               for (Int_t i=v_min_position_165; ; i++) {
                  if ((v_base_165 - board165_waveform[chn][i]) < sigma_v_base_165) {
                     t_stop_165 = board165_time[chn][i+1] * TMath::Power(10,9); //[ns]
                     v_stop_165 = i+1; // [V]
                     break;
                  }
               }
               ...
               */
               // TIME OF CHANNEL
               for (Int_t t=v_min_position_165; ; t--) { // LOOP ON SAMPLES OF WAVEFORM
                  if (board165_waveform[chn][t] > v_th_165) {
               ...
               if (1) {
                  ...
                  std::cout << "\n------------------------------------------" << std::endl;
                  std::cout << "\nt_start_165      [ns]   = " << t_start_165  << std::endl;
                  std::cout << "\nt_stop_165       [ns]   = " << t_stop_165   << std::endl;
                  std::cout << "\nv_start_165      [V]    = " << v_start_165  << std::endl;
                  std::cout << "\nv_stop_165       [V]    = " << v_stop_165   << std::endl;
      ...
      for (Int_t chn=0; chn<12; chn++) {
         ...
         if (board166_hit[chn] == 1) {
            ...
            Double_t sigma_v_base_166 = 0.;                    // V BASELINE (PEDESTAL) [V]
            ...
            Double_t v_start_166;                              // V START OF SIGNAL [V]
            Double_t v_stop_166;                               // V STOP OF SIGNAL  [V]
            Double_t v_min_position_166 = 0;                   // POSITION OF V PEAK
            Double_t time_166[16];                             // TIME OF WF OF CHANNEL [a.u.]
            Double_t delta_time_166 = 0.;                      // ∆ TIME OF WF OF CHANNEL L AND R [a.u.]
            ...
            if (status==1) {
               ...
               for (Int_t i=50; i<800; i++) { // LOOP ON SAMPLES OF WAVEFORM
                  if (board166_waveform[chn][i] < v_peak_166) {
                     ...
                     v_min_position_166 = i;
                     t_peak_166 = board166_time[chn][i] * TMath::Power(10,9);
                  }
               }
               ...
               // TIME/V START
               for (Int_t i=10; ; i++) {
                  if ((v_base_166 - board166_waveform[chn][i]) > 5 * sigma_v_base_166) {
                     t_start_166 = board166_time[chn][i-1] * TMath::Power(10,9); //[ns]
                     v_start_166 = i-1; // [V]
                     break;
                  }
               }

               // TIME/V STOP (v_min_position_166)
               for (Int_t i=v_min_position_166; ; i++) {
                  if ((v_base_166 - board166_waveform[chn][i]) < sigma_v_base_166) {
                     t_stop_166 = board166_time[chn][i+1] * TMath::Power(10,9); //[ns]
                     v_stop_166 = i+1; // [V]
                     break;
                  }
               }
               ...
               */
               // TIME OF CHANNEL
               for (Int_t t=v_min_position_166; ; t--) { // LOOP ON SAMPLES OF WAVEFORM
