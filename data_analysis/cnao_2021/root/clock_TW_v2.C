	...
	for (Long64_t jentry=0; jentry<1; jentry++)           // ONLY FIRST EVENT FOR THE MOMENT
   	{
		...
		{ 	
            if (voltage_TW_CLK[i] > zero_TW_CLK)
			{
				...
				// ZERO CROSSING POINT
				ZeroCrossingPoint[j]       = (zero_TW_CLK - b_fit_TW_CLK)/a_fit_TW_CLK; // [ns]
				sigma_ZeroCrossingPoint[j] = ZeroCrossingPoint[j] * sqrt(pow(sigma_a_fit_TW_CLK/a_fit_TW_CLK,2) + pow(sigma_b_fit_TW_CLK/b_fit_TW_CLK,2)); // Somma in quadratura degli errori sui parametri di best-fit
		...
		{	
			...
			new_sigma_ZeroCrossingPoint[i]   = sigma_ZeroCrossingPoint[i+1];
			...
			cout <<left<<setw(4)<< "*" <<left<<setw(10)<< i <<left<<setw(30)<<new_ZeroCrossingPoint[i]<<left<<setw(30)<<new_sigma_ZeroCrossingPoint[i]<<new_N_TW_CLK[i]<<left<<setw(10)<<""<<endl;
			cout << "**********************************************************************************************" << endl;
		}
		
		TGraph *gr_N_TW_CLK_vs_ZeroCrossingPoint = new TGraphErrors(new_n_point_TW_phi, new_N_TW_CLK, new_ZeroCrossingPoint, new_sigma_N_TW_CLK, new_sigma_ZeroCrossingPoint);
		TF1 *f_fit_N_TW_CLK_vs_ZeroCrossingPoint = new TF1("f_fit_N_TW_CLK_vs_ZeroCrossingPoint", "pol1", new_N_TW_CLK[0], N_TW_CLK[24]);
		...
		legend->AddEntry((TObject*)0, TString::Format("#chi^{2} / ndf = %.2f/%d",         f_fit_N_TW_CLK_vs_ZeroCrossingPoint->GetChisquare(),  f_fit_N_TW_CLK_vs_ZeroCrossingPoint->GetNDF()), "");
		legend->AddEntry((TObject*)0, TString::Format("#varphi_{CLK,TW} = %.3f #pm %.3f", f_fit_N_TW_CLK_vs_ZeroCrossingPoint->GetParameter(0), f_fit_N_TW_CLK_vs_ZeroCrossingPoint->GetParError(0)), "");
		legend->AddEntry((TObject*)0, TString::Format("T_{CLK,TW} = %.3f #pm %.3f",       f_fit_N_TW_CLK_vs_ZeroCrossingPoint->GetParameter(1), f_fit_N_TW_CLK_vs_ZeroCrossingPoint->GetParError(1)), "");	
