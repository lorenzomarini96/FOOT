...
// Author: L. Marini
// Macro to processing the data.root a make a root file containing a 
// tree.
///////////////////////////////////////////////////////////////////////
...
void rec::Loop()
{
	...
	Double_t TOF_9_X[nentries];
	Double_t TOF_9_Y[nentries];
	Double_t Q_9_X[nentries];
	Double_t Q_9_Y[nentries];
	...
	// CORRECTION (problem with the first element)
	...
	Double_t time_TOF = 0.;
	Double_t time_TW  = 0.;                     // TIME OF TOWFWALL: (T_bar_T_bar_layer + T_bar_front)/2
    ...
    Double_t v_peak_166;						  // V MINIMUM (PEAK) [V]
	...
	// START-COUNTER
	//*********************************************************
	Double_t time_mean_SC;
	Double_t time_sum_SC; 						  	// Per calcolo media ponderata del tempo (pesata sui valori di ampiezza)
	Double_t ampl_sum_SC; 						  	// Per calcolo media ponderata del tempo (pesata sui valori di ampiezza)
	Int_t    enablesum_SC;
	Double_t voltage_SC[8][1023];        	
	Double_t min_SC;                               	// V AMPLITUDE MIN OF CHANNEL [V]
    Double_t max_SC;                               	// V AMPLITUDE MAX OF CHANNEL [V]
	// INITIALIZE VALUES   
	//Double_t a_fit;
	//Double_t b_fit;
    ...
    Double_t v_peak_SC;   						      // V MINIMUM (PEAK)[V]
	...
	// CREATE ROOT FILE TO STORE A TREE
	TFile *f = new TFile("WF_analysis.C", "RECREATE");
	TTree *tree = new TTree("tree", "test tree");

	tree->Branch("time_TW",      &time_TW,     	"time_TW/D");
	tree->Branch("tof_9_X",      &tof_9_X,     	"tof_9_X/D");
	tree->Branch("tof_9_Y",      &tof_9_Y,     	"tof_9_Y/D");
	tree->Branch("q_9_X",        &q_9_X,        "q_9_X/D");
	tree->Branch("q_9_Y",        &q_9_Y,        "q_9_Y/D");
	tree->Branch("q_9_Y",        &q_9_Y,        "q_9_Y/D");
	tree->Branch("delta_CLK_X",  &delta_CLK_X,  "delta_CLK_X/D");
	tree->Branch("delta_CLK_Y",  &delta_CLK_Y,  "delta_CLK_Y/D");
	tree->Branch("time_mean_SC", &time_mean_SC, "time_mean_SC/D");
  	...
  	for (Long64_t jentry=0; jentry<nentries/2; jentry++)
   	{
		...
		// 1° CHECK SIGNAL
      	for (Int_t chn=0; chn<12; chn++) 
      	{
        	...
        	min_166 = board166_waveform[chn][100]; // V AMPLITUDE MIN OF CHANNEL [V]
        	max_166 = board166_waveform[chn][100]; // V AMPLITUDE MAX OF CHANNEL [V]
		...
		// LOOP ON CHANNELS OF WAVEDREAM
      	for (Int_t chn=0; chn<12; chn++) 
      	{
			...
			{
            	v_peak_166 = board166_waveform[chn][300]; // V MINIMUM (PEAK) [V]
		...
		//****************************************************************************
		time_mean_SC = 0;
		time_sum_SC  = 0; // Per calcolo media ponderata del tempo (pesata sui valori di ampiezza)
		ampl_sum_SC  = 0; // Per calcolo media ponderata del tempo (pesata sui valori di ampiezza)
		...
		enablesum_SC = 0;
     	...
     	// LOOP ON CHANNELS OF WAVEDREAM
      	...
      	{	
			...
			// 1° CHECK SIGNAL
        	// FIND MIN AND MAX OF WAVEFORM
        	min_SC = voltage_SC[chn][100]; // V AMPLITUDE MIN OF CHANNEL [V]
        	max_SC = voltage_SC[chn][100]; // V AMPLITUDE MAX OF CHANNEL [V]
			...
			{
            	// INITIALIZE VALUES   
            	v_peak_SC = voltage_SC[chn][300];        // V MINIMUM (PEAK)[V]
