	...
	for (Long64_t jentry=0; jentry<1; jentry++)           // ONLY FIRST EVENT FOR THE MOMENT
   	{
		...
		{	
			...
			{ 	
            	if (voltage_TW_CLK[i] > zero_TW_CLK)
				{
					...
					sigma_ZeroCrossingPoint[i] = ZeroCrossingPoint[j] * sqrt(pow(sigma_a_fit_TW_CLK/a_fit_TW_CLK,2) + pow(sigma_b_fit_TW_CLK/b_fit_TW_CLK,2)); // Somma in quadratura degli errori sui parametri di best-fit
		...
		TGraph *gr_N_TW_CLK_vs_ZeroCrossingPoint = new TGraphErrors(new_n_point_TW_phi, new_N_TW_CLK, new_ZeroCrossingPoint);
		...
		gr_N_TW_CLK_vs_ZeroCrossingPoint->GetXaxis()->SetRangeUser(0, 30);
